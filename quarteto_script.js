...
var word3 = wordList[Math.floor(Math.random() * wordList.length)].toUpperCase();
var word4 = wordList[Math.floor(Math.random() * wordList.length)].toUpperCase();
var word1SemAcento = removerAcentos(word1);
var word2SemAcento = removerAcentos(word2);
var word3SemAcento = removerAcentos(word3);
var word4SemAcento = removerAcentos(word4);
if(word1 == word2 && word1 == word3 && word1 == word4)
{
    word1 = wordList[Math.floor(Math.random() * wordList.length)].toUpperCase();
}
else if(word2 == word3 && word2 == word4)
    {
...
console.log(word3);
console.log(word3SemAcento);
console.log(word4);
console.log(word4SemAcento);
...
function start()
{
    ...
    for(let r = 0; r < height; r++)
    {
        ...
        {
            let tile = document.createElement("span");
            tile.id = "jogo3-" + r.toString() + '-' + c.toString();
            tile.classList.add("tile");
            tile.innerText = "";
    ...
    for(let r = 0; r < height; r++)
    {
        ...
        {
            let tile = document.createElement("span");
            tile.id = "jogo4-" + r.toString() + '-' + c.toString();
            tile.classList.add("tile");
            tile.innerText = "";
...
function update()
{
    ...
    for(let c = 0; c < width; c++)
    {
        ...
        //jogo 3
        currTile = document.getElementById("jogo3-" + row.toString() + '-' + c.toString());
        letter = currTile.innerText;
        tentativa3 += letter;

        //jogo 4
        currTile = document.getElementById("jogo4-" + row.toString() + '-' + c.toString());
        letter = currTile.innerText;
        tentativa4 += letter;
    }
    tentativa1 = tentativa1.toLowerCase();
    tentativa2 = tentativa2.toLowerCase();
    tentativa3 = tentativa3.toLowerCase();
    tentativa4 = tentativa4.toLowerCase();
    ...
    if(!gameOver3 && !wordListSemAcento.includes(tentativa3))
    {
        document.getElementById("resposta").innerText = "Palavra não aceita.";
        return;
    }
    ...
    {
        letter = word2SemAcento[i];
        if(letterCount3[letter])
        {
            letterCount3[letter] += 1;
        }
        else
        {
            letterCount3[letter] = 1;
    ...
    for(let i = 0; i < word4.length; i++)
        ...
        if(letterCount4[letter])
        {
             letterCount4[letter] += 1;
        }
        else
        {
            letterCount4[letter] = 1;
    ...
    for(let c = 0; c < width; c++)
    {
        ...
        let currTile3 = document.getElementById("jogo3-" + row.toString() + '-' + c.toString());
        let letter3 = currTile3.innerText;
        let currTile4 = document.getElementById("jogo4-" + row.toString() + '-' + c.toString());
        let letter4 = currTile4.innerText;
        ...
        if(!gameOver3)
        {
            if(word3SemAcento[c] == letter3)
            {
                currTile3.classList.add("correto");
                correto3 += 1;
                letterCount3[letter] -= 1;
            }    
            if(correto3 == width)
        ...
        if(!gameOver4)
        {
            if(word4SemAcento[c] == letter4)
            {
                currTile4.classList.add("correto");
                correto4 += 1;
                letterCount4[letter] -= 1;
            }    
            if(correto4 == width)
    ...
    for(let c = 0; c < width; c++)
    {
        ...
        if(!gameOver2 && !currTile2.classList.contains("correto")) {
            if(word2SemAcento.includes(letter2) && letterCount2[letter2] > 0) {
                ...
                let keyTile = document.getElementById("Key" + letter2);
                if(!keyTile.classList.contains("correto")) {
                    keyTile.classList.add("contem");
                }
        ...
        if(!gameOver3 && !currTile3.classList.contains("correto")) {
            if(word3SemAcento.includes(letter3) && letterCount3[letter3] > 0) {
                currTile3.classList.add("contem");
                let keyTile = document.getElementById("Key" + letter3);
                if(!keyTile.classList.contains("correto")) {
                    keyTile.classList.add("contem");
                }
                letterCount1[letter3] -= 1;
            } else {
                currTile3.classList.add("errado");
            }
        }
        if(!gameOver4 && !currTile4.classList.contains("correto")) {
            if(word4SemAcento.includes(letter4) && letterCount4[letter4] > 0) {
                currTile4.classList.add("contem");
                let keyTile = document.getElementById("Key" + letter4);
                if(!keyTile.classList.contains("correto")) {
                    keyTile.classList.add("contem");
                }
                letterCount4[letter4] -= 1;
            } else {
                currTile4.classList.add("errado");
                let keyTile = document.getElementById("Key" + letter4);
                if (!keyTile.classList.contains("correto") && !keyTile.classList.contains("contem")) {
                    keyTile.classList.add("disabled");
                }
            }
    ...
    // Verificar se as quatro palavras foram acertadas
    if (gameOver1 && gameOver2 && gameOver3 && gameOver4) {
        gameOver = true;
        document.getElementById("mensagem").innerText = "Parabéns!";
    }
    
    // Exibir palavras corretas se acabaram as tentativas e não acertou
    if (row == height && !gameOver) {
        gameOver = true;
        document.getElementById("palavrasCorretas").innerText = `${word1}, ${word2}, ${word3} e ${word4}`;
    }
}
